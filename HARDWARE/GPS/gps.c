...
void Ublox_Analasis(void)
{
	...
	if(USART1_RX_STA&0X8000)	
	{
		...
		for(i=0;i<rxlen;i++)
		{
			TEMP_BUF[i]=USART1_RX_BUF[i];
		}
 		USART1_RX_STA=0;		   	
		TEMP_BUF[i]=0;
		//printf("%s\n", TEMP_BUF);
		GPS_Analysis(&gpsx,(u8*)TEMP_BUF);
		Gps_Msg_Show();		
 	} 
}
...
void Gps_Msg_Show(void)
{
  ...
  __align(4) u8 dtbuf[50];
	...
	sprintf((char *)dtbuf,"Valid satellite:%02d",gpsx.posslnum);	 		
 	printf("%s\n", dtbuf);		    
	sprintf((char *)dtbuf,"Visible satellite:%02d",gpsx.svnum%100);	 		
 	printf("%s\n", dtbuf);			 
